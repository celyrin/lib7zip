...
#endif //__APPLE__
{
  ...
  if ( NULL == ( pTmpDir = opendir(szEntryName) ) )
  {
    if ( errno == ENOTDIR )
    {
      ...
      void * pHandler = dlopen(path.c_str(), RTLD_LAZY | RTLD_LOCAL);
...
HMODULE Load7ZLibrary(const wstring & name)
{
  ...
  HMODULE pHandler = dlopen(tmpName.c_str(), RTLD_LAZY | RTLD_LOCAL);
      ...
      lib_search_pathlistIt++)
  {
    ...
    pHandler = dlopen(path_prefix.c_str(), RTLD_LAZY | RTLD_LOCAL);
    if ( pHandler != NULL)
      break;
  }
  return pHandler;
